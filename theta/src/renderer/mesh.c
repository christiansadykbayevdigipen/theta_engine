...
void theta_mesh_init(theta_mesh* mesh, f32* vertices, u32 number_of_vertices, u32 dimension, u32* indices, u32 number_of_indices, f32* normals, u32 number_of_normals) {
    THETA_PROFILE();

    switch(theta_renderer_get_api()) {
    case THETA_API_OPENGL:
        theta_mesh_init_opengl(mesh, vertices, number_of_vertices, dimension, indices, number_of_indices, normals, number_of_normals);
        break;
    default:
...
void theta_mesh_init_quad(theta_mesh* mesh) {
    theta_mesh_init(mesh, g_quad_vertices, sizeof(g_quad_vertices) / sizeof(g_quad_vertices[0]), 3, g_quad_indices, sizeof(g_quad_indices) / sizeof(g_quad_indices[0]), NULL, 0);
}

void theta_mesh_init_cube(theta_mesh* mesh) {
    theta_mesh_init(mesh, g_cube_vertices, sizeof(g_cube_vertices) / sizeof(g_cube_vertices[0]), 3, g_cube_indices, sizeof(g_cube_indices)/sizeof(g_cube_indices[0]), g_cube_normals, sizeof(g_cube_normals)/sizeof(g_cube_normals[0]));
}