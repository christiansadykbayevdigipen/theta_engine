...
void theta_mesh_init(theta_mesh* mesh, f32* vertices, u32 number_of_vertices, u32 dimension, u32* indices, u32 number_of_indices) {
    THETA_PROFILE();

    switch(theta_renderer_get_api()) {
    case THETA_API_OPENGL:
        theta_mesh_init_opengl(mesh, vertices, number_of_vertices, dimension, indices, number_of_indices);
        break;
    default:
