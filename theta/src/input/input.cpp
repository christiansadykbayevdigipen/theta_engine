...
void theta_input_system_bind_input(theta_input_system* system, const char* tag, theta_input_layout layout, void (*input_callback)(theta_vector3f)) {
    ...
    system->inputs = theta_node_insert_at_head(system->inputs, &descriptor, sizeof(theta_input_descriptor));
}

void theta_input_system_on_key_down(theta_input_system* system, char key) {
    theta_node* temp = system->inputs;
    while(temp != NULL) { // Go through each input binding
        theta_input_descriptor descriptor = *(theta_input_descriptor*)temp->data;
